...
- `GET /health` -> `{ "status": "ok", "raagas": 70, "traditions": { "carnatic": 40, "hindustani": 30 } }`
- `POST /identify` (multipart `audio`) -> `{ top3, tonic_hz, heard_seconds, no_prediction, swara_activation[12] }` (each `top3` entry carries a `tradition`)
