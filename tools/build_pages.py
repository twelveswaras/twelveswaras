...
(function () {
  ...
  // A real sung "aah" (Freesound #764329 by ShangusBurger, CC0) tuned to Sa (F3) and pitch-shifted
  // per swara. Falls back to a formant-synth "aah" if the sample can't load or Web Audio is absent.
  var SA_HZ = 174.61, SAMPLE_HZ = 176.64, ctx = null, aaBuf = null, aaRaw = null, aaDecoding = false;
  try { fetch('../swara-aa.mp3').then(function(r){ return r.arrayBuffer(); })
          .then(function(b){ aaRaw = b; }).catch(function(){}); } catch(e){}
  function audio(){
    if(!ctx){ var AC=window.AudioContext||window.webkitAudioContext; ctx=AC?new AC():null; }
    if(ctx && aaRaw && !aaBuf && !aaDecoding){ aaDecoding=true;
      ctx.decodeAudioData(aaRaw.slice(0)).then(function(buf){ aaBuf=buf; }).catch(function(){}); }
    return ctx;
  }
